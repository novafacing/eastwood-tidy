...
void Rule11cCheck::registerMatchers(MatchFinder *Finder) {
  Finder->addMatcher(
      binaryOperator(isAssignmentOperator()).bind("binary_operator"), this);
}
        ...
        if (is_stop) {
          ...
          if (s_parent) {
            ...
            if (s_visitor.getAssignOpCount() > 1) {
              diag(s_parent->getBeginLoc(), "Expression contains more than one "
                                            "assignment.");
            }
            ...
            if (d_visitor.getAssignOpCount() > 1) {
              diag(d_parent->getLocation(), "Expression contains more than one "
                                            "assignment.");
            }
          }
