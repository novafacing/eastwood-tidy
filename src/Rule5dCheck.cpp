                ...
                if (auto MatchedDecl =
                    ...
                    if (FunctionDefinition) {
                        ...
                        SourceLocation StartOfFile = SM.getLocForStartOfFile(
                            SM.getFileID(FunctionDefinitionRange.getEnd()));
                            ...
                            if (ws_ct != 1 ||
                                std::string(
                                    ...
                                    SM.getCharacterData(ws_tokens.back().getEndLoc()))
                                        .size() > 1) {
                                if (SM.getSpellingLineNumber(
                                        ws_tokens.back().getLocation()) ==
                                    SM.getSpellingLineNumber(
                                        ws_tokens.back().getEndLoc())) {
                        ...
                        if (tok.getKind() == tok::comment) {
                            ...
                            if (comment_raw != "/* " + fname + "() */") {
                                diag(tok.getLocation(),
                                     std::string("End of function comment is "
                                                 "malformed. Got \"") +
                                         comment_raw + "\" Expected \"/* " + fname +
                                         "() */\".");
                                return;
                            } else {
                                return;
