                ...
                if (auto MatchedDecl = Result.Nodes.getNodeAs<FunctionDecl>("function_decl")) {
                    ...
                    if (FunctionDefinition) {
                        ...
                        while (!RawLexer.LexFromRawLexer(tok)) {
                            if (tok.getLocation() == FunctionDefinitionRange.getBegin()) {
                                /*std::cout << "Stopping lex at token |" << std::string(SM.getCharacterData(tok.getLocation()),
                                    SM.getCharacterData(tok.getEndLoc())) << "|" << std::endl; */
                                break;
                            }
                            tokens.push_back(tok);
                        }
                        /*std::cout << "Token range: |" << std::string(SM.getCharacterData(tokens.at(tokens.size() - 3).getLocation()),
                                    SM.getCharacterData(tokens.back().getEndLoc())) << "|" << std::endl; */
                        if (tokens.size() >= 3) {
                            std::vector<Token>::reverse_iterator rit = tokens.rbegin();

                            size_t comment_ct = 0;
                            for (size_t i = 0; rit != tokens.rend() and i < 3; rit++, i++) {
                            ...
                            // Make sure the last token is just a newline and the previous is a comment. Then, check
                            // for comment formatting
                            if (not isWhitespace(*SM.getCharacterData(tokens.back().getLocation()))) {
                                diag(tokens.back().getEndLoc(), "Empty line required.");
                                return;
                            }
                            if (not isWhitespace(*SM.getCharacterData(tokens.at(tokens.size() - 3).getLocation()))) {
                                diag(tokens.at(tokens.size() - 2).getLocation(), "Empty line required.");
                                return;
                            }
                            if (tokens.at(tokens.size() - 2).getKind() == tok::comment) {
                                ...
                                diag(tokens.at(tokens.size() - 2).getLocation(), "Malformed header comment.");
                                return;
                            }
                            diag(tokens.back().getEndLoc(), "Missing header comment.");
                            return;
