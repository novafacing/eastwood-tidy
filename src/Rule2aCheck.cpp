...
void Rule2aCheck::registerMatchers(MatchFinder *Finder) {
  ...
  Finder->addMatcher(translationUnitDecl().bind("translation_unit"), this);
}
...
void Rule2aCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  if (auto MatchedDecl = Result.Nodes.getNodeAs<FunctionDecl>("function")) {
    if (not this->checked) {
      ...
      Lexer RawLexer(StartOfFile, Context->getLangOpts(), File.begin(),
                     TokenBegin, File.end());
      ...
      while (!RawLexer.LexFromRawLexer(tok)) {
        ...
        if (raw_tok_data == "\n") {
          ...
          if (SM.getSpellingColumnNumber(tok.getLocation()) > 80) {
            diag(tok.getLocation(), "Line exceeds 80 characters.");
          }
        }
        if (tok.isAtStartOfLine()) {
          ...
          if (lines >= 2) {
            if (tq.at(tq.size() - 1).second == "\r") {
              ...
              // your source file.");
            } else if (std::regex_match(tq.at(tq.size() - 2).second, results,
                                        trailing_ws)) {
            ...
            if (lcol >= MAX_LINE_LEN) {
              diag(tq.at(tq.size() - 1).first.getLocation(),
                   "Lines must be <= " + std::to_string(MAX_LINE_LEN) +
                       " characters in length.");
            }
          }
          std::string Indentation =
              Lexer::getIndentationForLine(tok.getLocation(), SM).str();
    ...
    this->checked = true;
  } else if (auto MatchedDecl = Result.Nodes.getNodeAs<TranslationUnitDecl>(
                 "translation_unit")) {
  }
}
} // namespace eastwood
