...
//===----------------------------------------------------------------------===//
//
// Variable names should be descriptive and meaningful. Note: This only dumps
// variable names, it does not error
...
Rule1bCheck::Rule1bCheck::Rule1bCheck(StringRef Name, ClangTidyContext *Context)
    ...
    if (this->dump == "true") {
        for (auto type :
             {"variable", "function", "enum", "union", "struct", "field", "typedef"}) {
...
void Rule1bCheck::onEndOfTranslationUnit() {
    if (this->dump == "true") {
        ...
        std::cout << std::string(std::max(max_decl_len + 10, header.length()), '=')
                  << std::endl;
