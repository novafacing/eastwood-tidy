...
def create_contents(if_pass, section, subsection, spec):
    ...
    comment.append('#include "eastwood-rule{}{}.h"'.format(write_roman(int(section)), subsection.upper(), "pass" if if_pass else "fail"))
    return "\n".join(comment) + "\n" + MAIN_TEMPLATE
...
def format_new_test(if_pass, section, subsection, spec):
    ...
    if spec["test"]:
        test = {}
        test["filename"] = 'eastwood-rule{}{}'.format(write_roman(int(section)), subsection.upper(), "pass" if if_pass else "fail")
